package com.blez.basic_dictionary.feature_dictionary.presentation
...
class WordInfoViewModel @Inject constructor(val useCase: GetWordInfoUseCase) : ViewModel() {
                ...
                .onEach { result ->
                    when (result) {
                        is Resource.Error -> {
                            ...
                            _eventFlow.emit(UI_Event.ShowSnackbar(result.message ?: "Unknown"))
                        }
    ...
    sealed class UI_Event {
        data class ShowSnackbar(val message: String) : UI_Event()
